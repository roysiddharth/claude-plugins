...
If you have not already explored the codebase, do so to understand the current state of the code. Use the explorer subagent.
...
First, ensure both labels exist on the repo:

```bash
gh label create afk --color 0075ca --description "Ralph can implement this autonomously" 2>/dev/null || true
gh label create hitl --color e4e669 --description "Requires human interaction" 2>/dev/null || true
```
...
For HITL slices:
```bash
gh issue create --title "..." --body "..." --label hitl
```
